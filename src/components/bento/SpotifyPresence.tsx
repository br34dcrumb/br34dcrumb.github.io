...
const SpotifyPresence = () => {
  ...
  const fetchWithRetry = async (retryCount = 0): Promise<void> => {
    try {
      ...
      const response = await fetch(
        'https://lastfm-last-played.biancarosa.com.br/br34dcrumb/latest-song',
        {
  ...
  return (
    <>
      <div className="relative flex size-full flex-col justify-between gap-4 p-2">
        <div
          className="aspect-square min-h-0 max-w-[40%] flex-shrink border bg-cover bg-center grayscale sepia-50"
          style={{
      ...
      <div className="text-primary absolute top-2 right-2 m-3">
        <FaSpotify size={42} />
      </div>
      <a
        href={url}
        ...
        className="bg-border/50 text-primary ring-ring group/spotify-link absolute end-2 bottom-2 m-3 rounded-full p-3 transition-[box-shadow] duration-300 hover:ring-2 focus-visible:ring-2"
      >
        <MoveUpRight
          size={16}
